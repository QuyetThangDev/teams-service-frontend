  ...
  <div>
    <Card class="border border-gray-300 shadow-none">
      <CardHeader class="flex flex-col items-start gap-4 p-4 border-b">
        ...
        <CardDescription>
          Permanently remove your team and all of its contents from the Viot. This action is not
          reversible, so please continue with caution.
        </CardDescription>
      </CardHeader>
      <CardFooter class="flex justify-end px-6 py-4 bg-red-100 border-t">
        <Button variant="destructive" @click="onDelete">Delete Team</Button>
      </CardFooter>
    </Card>
...
const validationSchema = toTypedSchema(
  z.object({
    confirmDelete: z.string().refine((val) => val === 'delete my team', {
      message: 'You must type "delete my team" exactly to proceed.'
    })
  })
)
